      ...
      axios
        .get(
          "https://api.nasa.gov/mars-photos/api/v1/manifests/" +
            rover +
            "?api_key=qYavjcnrZOaoLo8ayJofWJx3duMuedGFLgffRNif"
        )
        .then(response => {
          console.log(
            response.data.photo_manifest.photos.filter(
              photo => photo.earth_date == date
            )[0]
          );
          this.results = response.data.photo_manifest.photos.filter(
            photo => photo.earth_date == date
          )[0];
        });
    },
